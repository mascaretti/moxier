...
`r colorise("SF: Insert description of the data", "red")`

```{r read-data, exercise=FALSE}
(df <- moxier::cleaneddata)
```

We build subsets of the dataset and show the data. Notice that R returns a `list` or a `data.frame` depending on how many columns we pick. We thus call the `data.frame()` function on the lists only for visualisation reasons.

```{r build-subset, exercise=FALSE}
data.frame(df_gender <- df[, 1])
data.frame(df_country <- df[, 2])
(df_bio <- df[, 3:5])
(df_social <- df[, 6:8])
```
...
*Exercise*
Visualise the data to get a grasp of the problem.

```{r visualise-data, exercise=TRUE}
pairs(df)
...
`r colorise("SF: Add comments on distances", "red")`
...
`r colorise("SF: Comment mean and variance", "red")`
...
`r colorise("SF: Why compute the quantities below? Are they useful?", "red")`
...
and we add the centroids (each centroid is the element the summarises and represent the properties of all the points belonging to the cluster!)
...
We print in a table the results according to gender and country of origin. This seemingly unimportant passage, it is actually fundamental in clustering: we are seeking a way to represent the data and to interpret them. We therefore have to constantly ask ourselves the question: "What is it that I have obtained? Is there a plausible interpretation of the results?"
...
For instance, we here see that in the first class, gender "female" is more represented than gender "male".


## Number of Clusters and Stability Check

`r colorise("Introduce WITHIN SS", "red")`


What happens if we change the number of clusters? Try to think about it. If we imagine to have one single huge cluster, we are ensuring stability above all. If we slightly move a point, nothing will change. Or if we start our algorithm from a different initialisation point, we will end up with the same result. Yet, as the number of clusters grows, we can expect more complex behaviours. Stability may become less certain.

Now, first of all, try to change the number of clusters and see what happens to the points.
...
And now we assess the stability of the clustering for different starting values.
...
`r colorise("Advantags wrt kmean", "red")`
...
```{r hierarchical-extract-clusters}
...
*Exercise* Try to set different numbers of clusters!
```{r hierarchical-set-clusters, exercise=TRUE}
no_clusters <- 4
  
  
clustera <- cutree(hclust(d=d, method='average'), no_clusters)
plot(df_bio[, 3:2], col=clustera+1, pch=16, asp=1)

clusterw <- cutree(hclust(d, method='ward.D'), no_clusters)
plot(df_bio[, 3:2], col=clusterw+1, pch=16, asp=1)
```
